...
- [A Defeasible Logic-based Framework for Contextualizing Deployed Applications](../Al-Anbaki_2019_DefeasibleLogicContextualizingApplications/notes.md) — Strong. Al-Anbaki et al. give a parallel-formalism answer to the same engineering problem this paper operationalises in Datalog/DL-Lite: how to let local context override global rules without losing consistency. Al-Anbaki et al.'s framework is in pure DL with the `±Δ / ±δ` proof tags but ships no execution backend, while this paper supplies exactly the kind of Datalog/ASP execution layer Al-Anbaki et al.'s `L = ⟨G, β, D, λ⟩` could plug into. Useful pair for propstore.
- [Defeasible Contextual Reasoning with Arguments in Ambient Intelligence](../Bikakis_2010_DefeasibleContextualReasoningArguments/notes.md) — Strong. Bikakis & Antoniou give an argumentation/P2P operationalisation of defeasible MCS over propositional contexts with per-context total preference orderings, while this paper gives a Datalog/ASP operationalisation of CKR-style defeasible context reasoning over DL-Lite_R. Both paper attach defeasibility to inter-context information flow but pick different execution substrates and different conflict-resolution disciplines (context-preference attack/defeat vs. clashing-assumption justifiable exceptions). Direct comparison point for any propstore work that wants to hold both options.
