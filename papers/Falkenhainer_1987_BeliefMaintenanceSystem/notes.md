...
- [[Sensoy_2018_EvidentialDeepLearningQuantifyClassification]] -- **Strong.** Both build on Dempster-Shafer theory for evidence combination: the BMS applies DS to TMS-style dependency networks with continuous belief propagation, while Sensoy applies DS to neural network outputs via the Dirichlet/subjective-logic bridge. Together they represent the symbolic (BMS) and connectionist (EDL) approaches to graded evidence tracking. The BMS could serve as the propagation substrate for EDL-produced belief masses.
- [[Josang_2001_LogicUncertainProbabilities]] -- **Strong.** Both build on Dempster-Shafer theory. The BMS uses DS belief intervals [s,p] with Dempster's rule for evidence combination within a TMS dependency network; Josang's subjective logic provides a richer opinion algebra (b, d, u, a) with logical operators (AND, OR, NOT) and evidential operators (consensus, discounting) that generalize Dempster's rule. Josang's consensus operator produces more intuitive results than Dempster's rule in Zadeh's paradox. The BMS's design being "semi-independent of the specific belief system" (p.71-72) suggests subjective logic opinions could replace DS intervals as an alternative belief formalism.
