...
- [Revision of Defeasible Logic Preferences](../Governatori_2012_RevisionDefeasibleLogicPreferences/notes.md) — **Strong.** Governatori et al. (2012) explicitly critiques the argument-level (whole-reasoning-chain) preference revision approach used by the DeLP-ATC line that Rotstein represents (Section 6, p.35-36): imposing preferences over whole arguments rather than single rules can produce the anomaly where an argument is warranted while all its sub-arguments are defeated. Governatori's rule-level superiority revision is the proposed antidote. Reading both side-by-side gives a complete view of the argument-level vs. rule-level debate in non-monotonic revision.
