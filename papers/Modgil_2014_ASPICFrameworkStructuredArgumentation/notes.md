...
1. **Dung's abstract argumentation treats arguments as atomic.** Abstract argumentation frameworks (Dung 1995) have no internal structure for arguments -- they are simply nodes in a graph. Real argumentation requires structured arguments built from premises and inference rules, with different types of attack targeting different components. *(p.31-32)*
2. **Prior structured approaches violate rationality postulates.** Caminada and Amgoud (2007) showed that existing structured argumentation formalisms can violate basic rationality requirements: their extensions may be inconsistent, or fail to be closed under strict rules. ASPIC+ was designed specifically to satisfy these postulates under well-defined conditions. *(p.32, p.49-50)*
3. **Pollock's system lacks explicit preferences and meta-variable schemes.** Pollock pioneered defeasible reasoning with argument graphs but his formalized defeasible rules do not have meta-variables ranging over classes of situations, and his system lacked explicit preference-based conflict resolution between competing arguments. *(p.50, p.59)*
4. **Vreeswijk's framework lacks undermining attacks and preferences.** Vreeswijk (1997) presented an argumentation-theoretic approach similar to Pollock's but uses only defeasible inference rules, with no undermining attacks on premises and no preference ordering for resolving conflicts. *(p.59)*
5. **Besnard and Hunter's classical-logic approach lacks undercutting and defeasible rules.** Their system uses only strict rules with deductive logic; it has no undercutting attacks (attacks on the applicability of inference steps) and no defeasible rules, limiting its expressiveness for modeling real-world defeasible reasoning. *(p.59-60)*
6. **Carneades uses different proof standards instead of Dung semantics.** While Carneades provides a structured approach, it uses proof standards rather than Dung's well-studied acceptability semantics, making it less well-connected to the foundational abstract argumentation theory. *(p.60)*
7. **Prior work conflated different types of attack.** Before ASPIC+, the distinction between undermining (attacking a premise), rebutting (attacking a conclusion), and undercutting (attacking an inference step) was not cleanly formalized in a single unified framework with clear preference-handling for each type. *(p.33-34, p.38)*
8. **Without transposition closure, consistency fails.** If strict rules are not closed under transposition (contraposition), arguments that should have counter-arguments may not, leading to inconsistent extensions. Prior frameworks did not systematically require this closure condition. *(p.45-46, p.49-50)*

## Design Rationale

1. **Bridge between abstract and structured argumentation.** ASPIC+ is designed to instantiate Dung's abstract framework with structured arguments, inheriting Dung's well-studied semantics (grounded, preferred, stable) while adding the internal argument structure that Dung's framework lacks. *(p.31-32, p.40-41)*
2. **Two key organizing principles.** The framework is built on two ideas: (1) conflicts between arguments are often resolved with explicit preferences, and (2) some premises only create a presumption in favour of their conclusion. This motivates the strict/defeasible rule distinction and necessary/ordinary premise distinction. *(p.31, p.35)*
3. **Three distinct attack types with different preference behavior.** Undermining (attacking premises), rebutting (attacking conclusions), and undercutting (attacking inference steps) are treated as fundamentally different. Crucially, undercutting attacks always succeed as defeats regardless of preferences, while undermining and rebutting are preference-dependent. This design choice reflects that questioning the applicability of an inference step is qualitatively different from opposing a conclusion. *(p.38-40)*
4. **Framework, not system.** ASPIC+ deliberately leaves open the choice of logical language, inference rules, knowledge base contents, and preference ordering. It is a framework for specifying argumentation systems, not a specific system. This maximizes flexibility for different application domains. *(p.31, p.57)*
5. **Transposition closure ensures consistency.** The requirement that strict rules be closed under transposition is a sufficient condition for satisfying the rationality postulates. This is a principled structural requirement rather than an ad hoc fix. *(p.45-46, p.49)*
6. **Two ordering principles for different domains.** Last-link ordering (comparing only the last defeasible rules) suits legal/normative reasoning where the final applicable rule is what matters. Weakest-link ordering (comparing all defeasible rules and premises) suits epistemic reasoning where the weakest component determines argument strength. *(p.42-44)*
7. **Argument schemes as defeasible rules.** Walton-style argument schemes are formalized as defeasible inference rules, with their critical questions mapping to the three attack types: assumption-challenging questions generate undermining attacks, conclusion-challenging questions generate rebutting attacks, and exception questions generate undercutting attacks. *(p.50-53)*
8. **Contrariness generalizes beyond classical negation.** The contrariness function allows asymmetric conflict relations (contraries vs contradictories), enabling modeling of assumption-based argumentation (ABA) and other formalisms where conflict is not simply classical negation. Contrary-based attacks always succeed like undercutting; contradictory-based attacks are preference-dependent. *(p.57-58)*
9. **Connection to Toulmin's model.** The framework maps naturally to Toulmin's argument structure: data = premises, warrant = defeasible rule, backing = support for the rule, rebuttal = attacking argument. This grounds ASPIC+ in the most widely-used informal argumentation model. *(p.53)*
...
- [Defeasible logic programming: DeLP-servers, contextual queries, and explanations for answers](../García_2014_DefeasibleLogicProgrammingDeLP-servers/notes.md) — **Strong.** DeLP and ASPIC+ are sibling structured-argumentation formalisms published in the same year. DeLP's strict/defeasible rule split (`L0 ← L1,…,Ln` / `L0 -< L1,…,Ln`), defeater taxonomy (proper / blocking), and dialectical-tree warrant procedure correspond closely to ASPIC+'s strict/defeasible rules, rebut/undercut/undermine attack types, and grounded-extension warrant. García & Simari additionally formalise contextual-query merge operators ⊕ ⊗ ⊖ over (Π, Δ) program pairs and δ-Explanation as marked dialectical trees — features ASPIC+ does not natively provide. Propstore's `propstore.aspic_bridge` implements ASPIC+; the García paper supplies the parallel DeLP-side reference for the same backward-chaining query semantics.
