...
The paper addresses the scalability problem of nanopublications -- small, provenance-carrying RDF packages with cryptographic identifiers -- showing that the overhead from auxiliary triples is amortized through incremental versioning and subset-based access. Evaluated on WikiPathways and DisGeNET life science datasets, the incremental approach outperforms traditional snapshots after just two versions. This is relevant to propstore because it validates the design pattern of content-hash-addressed, provenance-rich individual data units with version history -- the same pattern propstore uses for claims with git-backed storage.
