...
class TestGroundFactInterventions:
    ...
    def test_ground_fact_score_reflects_dependency_count(self) -> None:
        ...
        unused = _bird_yes_intervention(
            collect_ground_fact_interventions(unused_bundle)
        )
...
class TestGroundedRuleInterventions:
    def test_rule_with_n_substitutions_emits_n_grounded_rule_ids(self) -> None:
        ...
        assert {item.target.provenance.source_ids for item in ranked} == {
            ("rule:birds-fly",)
        }
        assert all(
            item.target.kind is InterventionKind.GROUNDED_RULE for item in ranked
        )
    ...
    def test_grounded_rule_score_increases_when_undercuttable(self) -> None:
        ...
        undercut = next(
            item
            for item in collect_grounded_rule_interventions(undercut_bundle)
...
class TestBridgeUndercutInterventions:
    def test_bridge_undercut_identities_stable_across_rebuilds(self) -> None:
        ...
        assert all(
            item.target.kind is InterventionKind.BRIDGE_UNDERCUT for item in first
        )
