...
class TestClaimStrengthConcrete:
    ...
    def test_larger_sample_stronger(self) -> None:
        a = claim_strength(_claim(sample_size=1000, uncertainty=0.2, confidence=0.8))
        b = claim_strength(_claim(sample_size=10, uncertainty=0.2, confidence=0.8))
        assert a.dimensions[0] > b.dimensions[0]

    def test_lower_uncertainty_stronger(self) -> None:
        a = claim_strength(_claim(sample_size=10, uncertainty=0.01, confidence=0.8))
        b = claim_strength(_claim(sample_size=10, uncertainty=0.5, confidence=0.8))
...
class TestClaimStrengthProperties:
    ...
    def test_claim_strength_always_three_non_negative_elements(self, claim) -> None:
        dims = claim_strength(_claim(**claim)).dimensions
        assert len(dims) == 3
