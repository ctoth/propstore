"""Lemon entry/form/sense identity laws: polysemy, homography, the form/dimension
boundary, and reference-stable entry identity.

Ported (behavioral) from the May-16 reference ``test_lemon_concepts.py`` +
``test_lemon_form_dimension_boundary.py``, repointed to ``propstore.core.lemon``;
DTO-shape assertions (the dimensions-module / form_utils scans) are dropped — the
forms/dimensions layer is Phase 2b.
"""
...
def test_dimensions_live_on_entry_not_form() -> None:
    entry = LexicalEntry(
        identifier="entry:temperature",
        canonical_form=LexicalForm(written_rep="temperature", language="en"),
        senses=(LexicalSense(reference=OntologyReference(uri="u:temperature")),),
        physical_dimension_form="temperature",
    )
    assert entry.physical_dimension_form == "temperature"
    assert not hasattr(entry.canonical_form, "physical_dimension_form")
