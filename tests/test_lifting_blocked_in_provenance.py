"""Phase 4 non-commitment: a BLOCKED lift stays recorded with provenance.

The reference asserted this through the worldline dependency tracker (Phase 7).
The Phase-4 surface contract is the one that must hold now: a lift blocked by an
authored exception is NOT dropped — it is projected into the
``lifting_materialization`` sidecar carrying its exception id and clashing set,
and only the render layer (status filtering) would hide it.
"""
...
def _blocked_system() -> LiftingSystem:
...
def test_blocked_lift_is_recorded_with_provenance_and_not_dropped(
    tmp_path: Path,
) -> None:
    system = _blocked_system()
    decisions = system.lift_decisions_between("ctx_source", "ctx_target", "claim-a")
    assert [d.status for d in decisions] == [LiftingDecisionStatus.BLOCKED]
    ...
    assert len(rows) == 1
    (row,) = rows
    assert row.status is LiftingDecisionStatus.BLOCKED
    assert row.exception_id == "except-alpha"
    assert row.clashing_set == ("claim-b",)
    assert row.justification == "authored clash"


def test_lifted_and_blocked_both_land_so_render_can_filter(tmp_path: Path) -> None:
    system = _blocked_system()
    blocked = system.lift_decisions_between("ctx_source", "ctx_target", "claim-a")
    ...
    schema = repo.build_sidecar(
        tmp_path / "side.db", lifting_decisions=blocked + lifted
    )
    rows = repo.render_materializations(tmp_path / "side.db", schema)
    assert sorted(r.status.value for r in rows) == ["blocked", "lifted"]
