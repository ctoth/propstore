...
def test_cli_promote_rules_selective_promotes_only_selected(tmp_path) -> None:
    ...
    assert repo.families.rules.load(RuleRef("rule-001")) is not None
    assert repo.families.rules.load(RuleRef("rule-003")) is not None
    assert repo.families.rules.load(RuleRef("rule-002")) is None
