...
def test_cli_promote_rules_selective_promotes_only_selected(tmp_path) -> None:
    ...
    assert repo.families.defeasible_rule.load("rule-001") is not None
    assert repo.families.defeasible_rule.load("rule-003") is not None
    assert repo.families.defeasible_rule.load("rule-002") is None
    tip = repo.git.branch_sha("proposal/rules")
    assert repo.families.proposal_rules.require(
        RuleProposalRef(PAPER, "rule-002"), commit=tip
    )
