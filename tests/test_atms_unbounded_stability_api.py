...
def _budget_exhausted_type() -> type[BaseException]:
    return atms_module.BudgetExhausted


def _flip_world(
    *,
    queryable_count: int = 12,
    flip_index: int = 8,
) -> tuple[BoundWorld, str, list[QueryableAssumption]]:
    ...
    return bound, bound.claim_status("claim_future").node_id, queryables
