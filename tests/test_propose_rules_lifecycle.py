"""Rule-extraction lifecycle (heuristic layer 3 → proposal branch only)."""
...
def test_propose_rules_writes_only_admitted_rule_proposals(
    ...
    assert proposal.extraction_provenance is not None
    assert proposal.extraction_provenance.prompt_sha == rule_extraction.PROMPT_SHA
    assert proposal.extraction_provenance.status == "stated"
    assert proposal.page_reference == "Ioannidis 2005 p. 0697"


def test_propose_rules_dry_run_does_not_commit(
    monkeypatch: pytest.MonkeyPatch, tmp_path
) -> None:
    from propstore.heuristic import rule_extraction
