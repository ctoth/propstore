...
def test_classify_pair_returns_unknown_when_detector_has_no_records(
    monkeypatch,
) -> None:
    monkeypatch.setattr(
    ...
    assert (
        merge_classifier._classify_pair(left, same_concept)
        is merge_classifier._DiffKind.UNKNOWN
    )
    assert (
        merge_classifier._classify_pair(left, different_concept)
        is merge_classifier._DiffKind.UNKNOWN
    )
