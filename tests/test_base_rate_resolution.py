"""Base-rate resolution over situated assertion identity.

A base rate is a sourced prior, not a bare float; a missing one is unresolved, not
``0.5`` (CLAUDE.md honest ignorance). The constructed opinion is paired with the
resolving profile's provenance via ``OpinionWithProvenance``.
"""
...
def test_construct_assertion_opinion_attaches_to_assertion_id() -> None:
    ...
    assert result.evidence_assertion_ids == (_aid("aarts_2015_replication_rate"),)
    # The opinion is the doxa.Opinion paired with the resolving profile's provenance.
    assert result.opinion.base_rate == 0.36
    assert result.opinion.opinion.a == 0.36
