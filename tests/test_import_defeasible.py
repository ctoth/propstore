...
def _manifest(status: ProvenanceStatus) -> ImportManifest:
    return ImportManifest(
        ...
        concepts=(
            ImportConceptRow(
                local_name="continuant",
                definition="A BFO continuant.",
                form="structural",
            ),
            ImportConceptRow(
                local_name="occurrent", definition="A BFO occurrent.", form="structural"
            ),
        ),
        claims=(
            ImportClaimRow(
...
def test_import_lands_on_source_branch_not_master(tmp_path: Path) -> None:
    ...
    assert {claim.source_local_id for claim in claims.claims} == {
        "claim_one",
        "claim_two",
    }
...
def test_defaulted_import_stays_defaulted(tmp_path: Path) -> None:
    ...
    assert (
        load_source_document(repo, _SOURCE).trust.status is ProvenanceStatus.DEFAULTED
    )
    provenance = read_provenance_note(repo.require_git().raw_repo, result.commit_sha)
    assert provenance is not None
...
def test_imported_claim_has_no_privileged_identity(tmp_path: Path) -> None:
    ...
    for claim in claims.claims:
        assert claim.artifact_id is not None and claim.artifact_id.startswith(
            "ps:claim:"
        )
