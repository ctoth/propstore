...
from propstore.core.labels import NogoodSet, assumption_label, make_environment_key
...
def test_ws_i_cycle_whose_external_support_is_nogood_is_nogood_pruned() -> None:
    ...
    engine = build_bound(claims=[]).atms_engine()
    ...
    engine._nodes[node_a] = ATMSAssumptionNode(
        ...
        label=assumption_label(assumption_a.assumption_id),
    )
    engine._nodes[node_b] = ATMSAssumptionNode(
        node_id=node_b,
        assumption=assumption_b,
        label=assumption_label(assumption_b.assumption_id),
    )
    ...
    engine.nogoods = NogoodSet(
        [make_environment_key(assumption_ids=("cycle_a", "cycle_b"))]
    )
