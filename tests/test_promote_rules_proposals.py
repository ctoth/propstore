...
from propstore.heuristic.rule_extraction import parse_atom
from propstore.proposal_promotion import UnknownProposalPath
from propstore.proposals_rules import RuleWorkflowError
...
def _seed_rule_proposals(monkeypatch: pytest.MonkeyPatch, repo: Repository) -> str:
...
def _seed_direct_rule_proposal(
    repo: Repository, *, paper: str, rule_id: str, predicate_id: str
) -> str:
    ...
    bound = repo.families.transact(
        message=f"Record rule proposal {rule_id}", branch=rule_proposal_branch()
    )
...
def test_promote_rule_proposals_selective_and_idempotent(
    ...
    promoted = repo.families.defeasible_rule.require("rule-001")
    assert promoted.source == PAPER
    assert promoted.rule_id == "rule-001"
    assert promoted.promoted_from_sha == proposal_sha
    assert repo.families.defeasible_rule.load("rule-003") is None
...
def test_plan_rule_proposal_unknown_id_raises(
    monkeypatch: pytest.MonkeyPatch, tmp_path
) -> None:
...
def test_generated_rule_proposal_promotion_rejects_undeclared_predicates(
    paper: str, rule_id: str, predicate_id: str
) -> None:
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        _seed_direct_rule_proposal(
            repo, paper=paper, rule_id=rule_id, predicate_id=predicate_id
        )
        ...
        assert repo.families.defeasible_rule.load(rule_id) is None
