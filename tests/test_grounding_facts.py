...
def test_concept_relation_emits_unary_fact() -> None:
    ...
    concepts = (
        ...
        ConceptRelations(
            concept_id="c2", canonical_name="rex", relationships=(("is_a", "Dog"),)
        ),
    )
...
def test_claim_role_about_output_target() -> None:
    registry = _registry(
        Predicate(
            predicate_id="about",
            arity=2,
            arg_types=("Claim", "Concept"),
            derived_from="claim.role:about",
        ),
        Predicate(
            predicate_id="outputs",
            arity=2,
            arg_types=("Claim", "Concept"),
            derived_from="claim.role:output",
        ),
        Predicate(
            predicate_id="targets",
            arity=2,
            arg_types=("Claim", "Concept"),
            derived_from="claim.role:target",
        ),
    )
    claim = Claim(
        claim_id="cl1",
...
def test_claim_context_fact() -> None:
    registry = _registry(
        Predicate(
            predicate_id="in_ctx",
            arity=2,
            arg_types=("Claim", "Context"),
            derived_from="claim.context",
        )
    )
...
def test_claim_condition_matches_authored_condition() -> None:
    registry = _registry(
        Predicate(
            ...
            arity=1,
            arg_types=("Claim",),
            derived_from="claim.condition:t>0",
        )
    )
...
def test_derived_from_none_contributes_nothing() -> None:
    registry = _registry(
        Predicate(predicate_id="bare", arity=1, arg_types=("Concept",))
    )
    concepts = (
        ConceptRelations(
            concept_id="c1", canonical_name="x", relationships=(("is_a", "Bird"),)
        ),
    )
    facts = extract_facts(GroundingFactInputs(concepts=concepts), registry)
    assert facts == ()
