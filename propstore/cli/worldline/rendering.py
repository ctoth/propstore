...
Pure text formatters over the reconstructed worldline compute forms
(:class:`~propstore.worldline.query.WorldlineResult` and the result-value types);
they hold no owner behaviour, only presentation.
"""
...
def sensitivity_lines(result: WorldlineResult) -> Iterable[str]:
    ...
    for concept, outcome in result.sensitivity.targets.items():
        if outcome.error is not None:
            yield f"{concept}: ERROR — {outcome.error.value}"
            continue
