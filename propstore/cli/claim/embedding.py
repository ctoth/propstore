...
@click.option(
    "--model",
    required=True,
    help="litellm model string, or 'all' for every registered model",
)
@click.option("--batch-size", default=64, type=int, help="Claims per API call")
@click.pass_obj
def embed(
    obj: dict, claim_id: str | None, embed_all: bool, model: str, batch_size: int
) -> None:
...
@click.pass_obj
def similar(
    obj: dict, claim_id: str, model: str | None, top_k: int, agree: bool, disagree: bool
) -> None:
    ...
    for hit in report.hits:
        emit(
            f"  {hit.distance:.4f}  {hit.claim_id}  [{hit.source_paper}]  {hit.summary[:120]}"
        )
