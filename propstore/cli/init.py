...
def _seed_proto_role_bundle(slot: dict) -> dict[str, object] | None:
...
def _seed_description_kind(entry: dict) -> dict[str, object] | None:
...
def _seed_qualia(entry: dict) -> dict[str, object] | None:
...
def _seed_concept_payload(entry: dict) -> dict[str, object]:
    name = str(entry["name"])
...
def _seed_concept_documents(repo: Repository) -> list[tuple[ConceptFileRef, ConceptDocument]]:
    ...
    package_concepts_dir = Path(__file__).resolve().parent.parent / "_resources" / "concepts"
    ...
    for seed_path in sorted(package_concepts_dir.glob("*.yaml")):
        ...
        if not isinstance(entries, list):
            raise click.ClickException(f"{seed_path} must contain a concepts list")
        for entry in entries:
            if not isinstance(entry, dict):
                raise click.ClickException(f"{seed_path} contains a non-mapping concept entry")
