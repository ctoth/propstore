"""``pks web`` — serve the read-only propstore web UI.

A thin launcher: it resolves the active repository, builds the FastAPI app via
:func:`propstore.web.app.create_app`, and hands it to uvicorn. The web package
is imported lazily inside :func:`create_web_app` so the root CLI stays lazy and
``pks --help`` never pulls FastAPI in. The public-bind refusal keeps the
no-auth read-only server on loopback unless the operator explicitly accepts the
risk with ``--insecure``.
"""
...
def web(
    ...
    repository_root = require_repo(obj).root
    app = create_web_app(repository_root=repository_root)
    url = _display_url(host, port)
    if _is_public_bind(host):
        click.echo(f"WARNING: serving propstore with no auth on {host}.", err=True)
...
def create_web_app(*, repository_root: "Path") -> "FastAPI":
