"""Branch / document / materialize views over a repository's git store.

``RepositorySnapshot`` is the read + materialize surface over the committed git
state: enumerate branches, read a typed document at a branch/commit, and
materialize a commit's semantic tree into the working directory. Materialize is
scoped to the semantic family init roots and honours the repository git policy's
ignored paths, so derived sidecar files on disk are never clobbered or deleted.
"""
...
class RepositorySnapshot:
    ...
    def materialize(
        ...
        *,
        ...
        head_guard: AbstractContextManager[object] = (
