...
:func:`query_fragility` binds a :class:`~propstore.world.WorldQuery` to an
:class:`~propstore.core.environment.Environment` and hands the bound world to
:func:`rank_fragility`, which collects every enabled intervention family, applies
a :class:`~propstore.fragility_types.RankingPolicy`, and returns a
:class:`~propstore.fragility_types.FragilityReport`. The orchestrator owns no
scoring of its own (CLAUDE.md CLI/owner discipline): every score comes from the
contributor and scoring modules over the world substrate, and the report is a
render-time view, never a stored commitment.
"""
