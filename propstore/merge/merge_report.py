...
These read a :class:`RepositoryMergeFramework` and project it into a serializable
report: the partial-AF relation partition, the completion count, and the credulous /
skeptical acceptance sets under a chosen semantics. The acceptance math is the
argumentation package's (CLAUDE.md substrate boundary) — this module queries it, it
...
def semantic_candidate_details(merge: RepositoryMergeFramework) -> list[dict[str, Any]]:
                ...
                "arguments": [
                    {
                        ...
                        "source_paper": argument.claim.paper,
                    }
    ...
    *,
    ...
    for argument in merge.arguments:
        detail: dict[str, Any] = {
            ...
            "branch_origins": list(argument.branch_origins),
            "provenance": argument.claim.provenance_mapping(),
    ...
    canonical_groups_out = {
        canonical_id: sorted(assertion_ids)
        for canonical_id, assertion_ids in sorted(canonical_groups.items())
    }
