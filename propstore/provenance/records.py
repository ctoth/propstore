...
These frozen records are the *out-of-band* provenance carriers used by the
import/PROV-O boundary: a source version, a license, an import run, a projection
frame, an external statement, and an external inference. Each validates its URI
and hash invariants on construction (Carroll et al. 2005 names a provenance
graph; the identifiers it names must be real URIs) and exposes a deterministic
``identity_payload`` so equal records compare equal. ``to_payload`` is the
one-way dict projection consumed by :mod:`propstore.provenance.prov_o`.

There is exactly one :class:`ProjectionFrameProvenanceRecord` spelling in the
codebase — this one. The argumentation projection boundary
(:mod:`propstore.structured_projection`, :mod:`propstore.aspic_bridge`) holds it
as a frozen-dataclass field; it is never nested inside a charter document, so it
does not need a msgspec mirror.
"""
