...
Each ``collect_*`` reads one substrate surface and emits ranked interventions:
the ATMS family scores assumptions by the live witness derivative
(provenance-semiring) behind a concept's stability; the discovery family scores
unmeasured inputs by downstream reach; the conflict family scores claim pairs by
the grounded-extension delta; and the grounding/bridge families score grounded
facts, grounded rules, and bridge undercuts off the ASPIC+ projection of a
grounded-rules bundle.

Grounding seam (CLAUDE.md substrate boundary): the gunray *complement encoder*
...
from argumentation.structured.aspic.aspic import Rule, top_rule
...
from propstore.aspic_bridge.extract import extract_justifications, extract_stance_rows
from propstore.aspic_bridge.grounding import project_grounded_rules, typed_scalar_key
from propstore.aspic_bridge.translate import StanceInput
from propstore.core.active_claims import ActiveClaim
from propstore.core.analyzers import shared_analyzer_input_from_graph
...
def _typed_row_key(row: Sequence[object]) -> str:
    return json.dumps(
        [typed_scalar_key(item) for item in row],
        sort_keys=True,
        separators=(",", ":"),
    )
...
def _decode_grounded_predicate(predicate_id: str) -> tuple[str, bool]:
    """Split a section predicate id into ``(predicate_name, negated)``.
...
def _parameterizations_to_queryables(
    ...
    for parameterization in bound.store.all_parameterizations():
        for condition in parameterization.conditions:
            queryables.append(str(condition))
...
def collect_assumption_interventions(
    ...
    *,
    ...
    for queryable_id, entry in sorted(contributions.items()):
        ...
        support_evidence = entry.support
...
def collect_missing_measurement_interventions(
    ...
    for parameterization in bound.store.all_parameterizations():
        ...
        if concept_filter and output_concept_id not in concept_filter:
            continue
        for input_id in parameterization.input_concept_ids:
            input_concept_id = str(input_id)
            if bound.active_claims(input_concept_id):
                continue
            downstream_by_input.setdefault(input_concept_id, set()).add(output_concept_id)
            parameterizations_by_input.setdefault(input_concept_id, set()).add(output_concept_id)
...
def collect_conflict_interventions(
    ...
    try:
        framework = shared_analyzer_input_from_graph(active_graph).argumentation_framework
    except Exception as exc:
        warnings.warn(
...
def collect_ground_fact_interventions(
...
def collect_grounded_rule_interventions(
    bundle: GroundedRulesBundle,
) -> tuple[RankedIntervention, ...]:
    projection = project_grounded_rules(bundle, {})
...
def collect_bridge_undercut_interventions(
    ...
    stance_rows: Sequence[StanceInput],
) -> tuple[RankedIntervention, ...]:
...
def build_bound_bridge_inputs(
    bound: FragilityWorld,
) -> tuple[tuple[ActiveClaim, ...], list[CanonicalJustification], list[StanceInput]]:
    active_claims = tuple(bound.active_claims())
    active_by_id = {str(claim.claim_id): claim for claim in active_claims}
    active_graph = bound.active_graph
    stance_rows = extract_stance_rows(bound.store, active_by_id, active_graph=active_graph)
    ...
    return active_claims, list(justifications), list(stance_rows)
