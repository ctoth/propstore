...
Every imported assertion is a *defeasible* claim with provenance — never truth.
The compiler attaches an import-provenance graph reference and carries the
external statement's attitude (Carroll 2005); it never privileges the source or
collapses two candidates into one identity. Equivalence between candidates is
recorded as an explicit witness (:class:`EquivalenceWitnessStore`) without any
sameAs-style identity closure (CLAUDE.md non-commitment; [[feedback_imports_are_opinions]]).
"""
