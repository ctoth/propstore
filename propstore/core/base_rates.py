...
A subjective-logic opinion's base rate ``a`` is a *prior*, and a prior is itself a
sourced assertion — not a bare float a caller may invent. This module resolves the
base rate for a target assertion from authored :class:`BaseRateProfile` records
(with stratified, cycle-rejecting dependencies) and only then constructs an
opinion, pairing it with the resolving profile's provenance.

Honest ignorance (CLAUDE.md): when no profile resolves, the result is an explicit
:class:`BaseRateUnresolved` — never a fabricated ``a = 0.5``. The opinion itself
is ``doxa``'s canonical type; its provenance rides alongside as an
:class:`~propstore.opinion_provenance.OpinionWithProvenance` pairing.
"""
...
class AssertionOpinion:
    """An opinion attached to a situated assertion, with its base-rate lineage.

    :attr:`opinion` is the :class:`~propstore.opinion_provenance.OpinionWithProvenance`
    pairing — a ``doxa.Opinion`` plus the provenance of the resolving base-rate
    profile — so the base rate's origin travels with the value.
    """

    assertion_id: AssertionId
    opinion: OpinionWithProvenance
...
def construct_assertion_opinion(
    *,
    ...
    """Resolve the base rate, then construct the opinion paired with provenance.

    Returns :class:`BaseRateUnresolved` (honest ignorance) when no profile
    resolves — the opinion is never built on a fabricated base rate.
    """
    ...
    carried = provenance if provenance is not None else resolved.provenance
