"""Allen interval relations over description-claim temporal anchors.

Allen's (1983) thirteen interval relations are evaluated by REDUCING each to a
condition over four timepoint variables (the two intervals' endpoints) and
discharging it through the ``condition-ir`` substrate: the timepoints are
``KindType.TIMEPOINT`` concepts (mapped to ``z3.Real``, semantically distinct
from QUANTITY), the relation is a CEL expression, and satisfaction is decided by
``condition_ir.ConditionSolver`` over Z3. propstore does NOT reimplement temporal
or Z3 logic; it consumes condition-ir's own types directly (``CelExpr``,
``ConceptInfo``, ``KindType``, ``ConditionSolver``, ``check_condition_ir``) — the
boundary is the import, not an adapter.
"""
...
class DescriptionTemporalAnchor(msgspec.Struct, frozen=True, forbid_unknown_fields=True):
    """The validity interval of a description claim, with provenance."""
...
def description_temporal_relation(
    ...
    """Decide an Allen ``relation`` between two anchors via condition-ir/Z3."""
