"""Branded algorithm-stage strings used on the canonical/runtime claim path.

An algorithm claim carries a free-form ``stage`` label (e.g. ``"preprocess"``).
It is a :class:`~typing.NewType` over ``str`` so the type checker keeps it
distinct from an arbitrary string without introducing a wrapper class.
"""
