...
Executes the contiguous pipeline the registry sliced, threading the value from
stage to stage. It re-checks the stage contract per pass (defence in depth), and
short-circuits to a failed :class:`PipelineResult` the moment a pass returns no
output together with error diagnostics — the quarantine seam: a pass may decline
to produce a CHECKED value without raising, and the caller records the
diagnostics rather than aborting the whole build.
"""
...
from propstore.semantic_passes.types import PassDiagnostic, PipelineResult, StageId
