...
The registry collects pass classes per family and, on demand, slices a
contiguous :class:`~propstore.semantic_passes.types.FamilyPipeline` from a start
stage to a target stage. It rejects duplicate pass names and any pass whose
``input_stage`` does not continue the chain, so a malformed pipeline fails at
registration / selection rather than mid-run.
"""
