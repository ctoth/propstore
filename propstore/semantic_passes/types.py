...
A *pass* takes a value at one ``StageId`` and returns it (or a transformed
value) at the next stage, together with diagnostics. Passes for a single
:class:`~propstore.families.registry.PropstoreFamily` form a linear pipeline; the
registry checks the stage contract is contiguous and the runner enforces it at
execution time. This is the one AUTHORED -> CHECKED framework shared by both
``pks build`` and ``pks validate`` (they differ only in terminal sink).
"""
