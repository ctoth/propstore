...
def concept_name(world: WorldStore, concept_id: ConceptId | str) -> str:
    concept = world.get_concept(str(concept_id))
    return concept.canonical_name if concept is not None else str(concept_id)
...
def display_claim_id(world: WorldStore, claim_id: str | None) -> str | None:
    if claim_id is None:
        return None
    # Charter claim identity *is* the display id (e.g. ``ps:assertion:...``); the
    # lookup confirms existence but the canonical id is what we surface.
    return claim_id
...
def pre_resolve_conflicts(
    ...
    from propstore.parameterization import reachable_concepts
    from propstore.world.resolution import resolve

    def _input_concepts(concept_id: str) -> list[str]:
        inputs: list[str] = []
        for edge in context.world.parameterizations_for(concept_id):
            inputs.extend(str(input_id) for input_id in edge.input_concept_ids)
        return inputs
    ...
    for cid in needs_check:
        ...
        trace.record_claim_dependencies(list(resolved.claims))
        trace.record_step(
