...
1. **Concept/semantic layer** — What things mean, how linguistic expressions map to formal concepts, how structured concept content and contexts qualify truth. The implemented concept artifact boundary is grounded in ontology lexicalization (lemon/Buitelaar 2011 and OntoLex-Lemon): authored concepts use `OntologyReference`, `LexicalEntry`, `LexicalForm`, and `LexicalSense`, with dimensions kept on the entry side rather than inside lexical forms. Qualia/proto-roles, first-class contexts, and micropublications are WS-A follow-on phases, not current implementation.
...
1. **Source-of-truth storage** — Claims, concepts, forms, contexts, conditions, provenance. Immutable except by explicit user migration. Never mutated by heuristic or LLM output. `propstore.storage` provides git-backed storage, branch isolation, formal merge-framework construction, and two-parent storage merge commits. ATMS lives in `propstore.world`, ASPIC+ lives in `propstore.aspic_bridge`, and IC merge operators live in `propstore.world.ic_merge`.
2. **Concept/semantic layer** — Concepts are authored as lemon-shaped links between linguistic entries and ontology references. `LexicalSense.reference` is functional; one entry may carry multiple senses for polysemy; distinct entries may share a written form for homography. Physical dimensions live on `LexicalEntry.physical_dimension_form` and in `propstore.dimensions`, not on `LexicalForm`. CEL + Z3 provide type-checking and condition reasoning. `KindType.TIMEPOINT` maps to `z3.Real` (same as QUANTITY) but is semantically distinct — not valid for parameterization or dimensional algebra. Vocabulary reconciliation uses lemon entry/form/reference identity and proposal artifacts, not string-token Jaccard.
