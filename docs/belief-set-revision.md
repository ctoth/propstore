...
`propstore.support_revision.realization`, `iterated`, `dispatch`, and the
world-bound revision methods do not own formal AGM kernels. They consume adapter
decisions, realize them over Propstore support/worldline objects, and keep the
result split into:
