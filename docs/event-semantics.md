...
- **Temporal order is a happens-before partial order (Lamport 1978) with declared-frame TIMEPOINT projections.** The primitive is `HappensBeforeEdge` — a provenance-bearing *posit* that one description strictly precedes another (a message receive, a human "X then Y", a cross-frame sync point), never a global fact. Cross-frame order is derived from the transitive closure of such evidence, answered by `temporal_order`. Within one declared `TemporalFrame`, a `DescriptionTemporalAnchor` carries *optional* endpoint bounds — an absent bound is honest ignorance, not a fabricated coordinate — and `description_temporal_relation` decides Allen-1983 interval relations three-valued (HOLDS / FAILS / UNDECIDED) through `KindType.TIMEPOINT` + `ConditionSolver`, evaluating satisfiability given partial knowledge. The single global real line is gone: bounds are comparable ONLY within one frame, so the same now-slice `docs/event-semantics.md` deflates at the event level is not re-installed one level below it. Concurrency and ignorance are distinct verdicts, and so are the two *bases* for concurrency — `CONCURRENT_PROVEN` (positive bounds proof; monotone — new edges can only contradict it, surfacing as `CONFLICTED`, never silently flipping it) vs `CONCURRENT_ASSUMED` (silence re-read under a per-query `assume_complete` declaration — the vector-clock/Lamport completeness assumption made explicit and auditable, like a render-policy assumption; defeasible, one new edge destroys it). Every `HappensBeforeEdge` declares its account, and transitive chaining is licensed only by mechanism-backed accounts (`message`, `synchronization`) — a merely `stated` order witnesses exactly its own endpoints, mirroring `causal_transitivity_allowed`'s refusal to chain stated causal links: two tellings composed would assert an order neither teller committed to. Verdicts return inside a `TemporalOrderJudgment` carrying the witnessing `OrderingLink` paths, so coordinate-grounded and posited arrows stay visibly different kinds, including inside a `CONFLICTED`. The judgment is recomputed per query from live anchors and edges; **nothing ever persists a derived order** — storage holds only authored edges, so revision that defeats an edge reaches every verdict that leaned on it at the next query. This is a thin Allen/happens-before layer over the substrate, not a second temporal system.
...
**The aboutness bill (2026-07-21 review).** If reference is a moving extension — coreference recomputed per query, under per-query semantics — what happens to a persisted claim that "referred to" a cluster when revision withdraws that cluster? Does its aboutness silently change, or dangle until the next render? The answer has two parts, and the first is structural: **dangling reference is inexpressible in storage by construction.** Clusters have no identity — `CoreferenceQuery.clusters()` returns anonymous sets of description-claim ids, nothing mints a cluster id, and no charter can carry one. Every stored artifact that relates descriptions (`CoreferenceMergeArgument.supports`, `CausalConnectionAssertion.cause_description_id`) references *claim ids*, which are rigid and stored. A stored claim cannot silently become "about something else" because the store has no vocabulary for being about a cluster at all; the deflation is total, so there is nothing to dangle. Second, the residue — the part of the objection that survives — is the *human authoring boundary*: a user renders, sees a cluster, and authors a new artifact "about that event." The system forces the artifact to lower to claim ids, but the *premise* (the cluster verdict the author was looking at) is otherwise lost. The committed answer: an artifact authored against a render verdict records that verdict as an **assumption in its provenance** — semantics used, merge-argument set, cluster membership — following the micropublication materialization pattern (`world/atms.py`: each micropub node is justified by its context node + claim nodes in a context-labeled environment). A withdrawn cluster is then a defeated assumption, and ATMS relabeling names every artifact premised on it — aboutness-revision falls out of existing machinery, with no stored referent and no new subsystem. There is deliberately no worldline *of referents*; there is a worldline of *premises*, which is all a descriptivist store is entitled to. Implementation lands with the first consumer that authors against cluster verdicts; until then this paragraph is the contract.
...
- **Lamport 1978, *Time, Clocks, and the Ordering of Events in a Distributed System*** — `to fetch` (temporal-layer primary). Happens-before partial order; total order as a per-observer projection, not a global fact — the primitive the rebuilt temporal layer implements as `HappensBeforeEdge` + declared-frame TIMEPOINT projections.
