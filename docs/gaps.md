...
- D-23 / no orphan sidecar rows — sidecar writes queued inside the head-bound transaction flush only after kernel CAS success; stale promotion attempts do not leave blocked mirror rows or diagnostics. Evidence: commits `77d67c3e`, `38b19aa1`, `9e3491b6`; test `tests/test_cas_rejection_no_orphan_rows.py`.
