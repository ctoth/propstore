...
- **Z3 three-valued results collapse to `bool` everywhere.** `propstore/z3_conditions.py:442-444, 459-463, 481, 489` map `solver.check() == z3.sat/unsat/unknown` into two-valued returns. Every downstream conflict-class decision, context-activation decision, and IC-merge validity check silently maps unknown to a plausible-looking Boolean outcome. No timeouts are ever set (`timeout|set_param|set_option` → zero hits across `propstore/`). Citation: axis-5 Findings 2.1-2.3; axis-6 item 4. Plan: WS-Z-types phases 1-2.
...
- **Semantic substrate remains incomplete beyond the implemented provenance and lemon slices.** WS-A Phase 1 added typed provenance and the git-notes named-graph carrier; WS-A Phase 2 added lemon-shaped concept artifacts, lemon core types, dimension/form separation, and Jaccard-free alignment. The remaining axis-3d semantic gap is now the unimplemented Phase 3/4 substrate: Pustejovsky qualia, Dowty proto-roles, description-kinds, McCarthy/Guha contexts and lifting, and Clark micropublications. Citation: axis-6 item 3; axis-3d; WS-A progress log. Plan: WS-A phases 3-4.
...
- **IC-merge candidate enumeration uses `product(observed_values)`, not `μ`-models.** `propstore/world/ic_merge.py:312-350` — candidates are restricted to values already in the sources. A true Konieczny IC merge considers all μ-models; sources `x=5` and `x=10` will never admit `x=7` even when μ allows. Module docstring is more honest than CLAUDE.md. Citation: axis-6 declared-limitation 3 "understates"; axis-3c. Plan: WS-B (belief-set layer).
...
- **CLAUDE.md mislocates ATMS, ASPIC+ bridge, and IC merge as owned by `propstore/repo/`.** Line 15 claim: "`propstore/repo/` provides … branch reasoning (ATMS/ASPIC+ bridge), and IC merge operators." Reality: ATMS lives in `propstore/world/atms.py`; ASPIC+ bridge is `propstore/aspic_bridge/` (package, not file); IC merge is `propstore/world/ic_merge.py`. `propstore/repo/` imports none of these. Citation: axis-9 Finding A.2; axis-6 item 12. Plan: CLAUDE.md rewrite (phase beyond WS-Z-gates).
