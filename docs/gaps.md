...
- **Semantic substrate has zero structural implementation.** Grep across `propstore/` for `qualia|telic|agentive|constitutive|proto_role|lemon|LexicalEntry|lexicon|sameAs|ist\(c|lifting|bridge_rule|LocalModels` returns zero hits. `frame` occurs only in `opinion.py` as Dempster-Shafer "frame of discernment," not Fillmore. Four papers named in CLAUDE.md (Fillmore 1982, Pustejovsky 1991, Buitelaar 2011, McCarthy 1993) are not cashed out. Citation: axis-6 item 3; axis-3d. Plan: WS-A (semantic substrate).
