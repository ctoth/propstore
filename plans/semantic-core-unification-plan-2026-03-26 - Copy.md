...
- [ ] Add or expand parity tests for `bind -> active_claims -> value_of -> derived_value -> resolved_value`.
- [ ] Add parity tests for `claim_graph`, `praf`, and `atms` over the same fixture family.
- [ ] Add parity tests for hypothetical overlays under each currently supported backend path.
- [ ] Add parity tests for worldline materialization and round-trip.
- [ ] Add parity tests for active-view conflict recomputation.
- [ ] Add property tests for binding-order invariance.
- [ ] Add property tests for environment/label normalization and antichain minimality.
...
- [ ] add a new properties-focused test file if the current suite needs separation

### Completion Criteria

- [ ] New tests fail before implementation adjustments.
- [ ] All tests pass after only compatibility-preserving fixes.
- [ ] The suite clearly exposes the current hypothetical `ATMS` downgrade behavior as a documented compatibility fact.

### Commit

- [ ] Commit: `test: lock world, backend, overlay, and worldline semantics`
