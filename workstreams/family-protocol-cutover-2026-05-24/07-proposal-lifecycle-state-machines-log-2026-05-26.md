...
## Iteration 4 - `rule proposal document surface`
...
- `RuleProposalDocument`
  - Disposition: delete
  - Owner after cleanup: `AuthoredRuleProposalArtifact`, generated by
    `AUTHORED_RULE_PROPOSAL_CHARTER`.
  - Action: Removed the old proposal-document token from code, tests, and the
    semantic contract manifest while keeping the generated charter-backed
    artifact behavior.

Gate results:
- Pass: `rg -n -F -- "RuleProposalDocument" propstore tests`
- Pass: `uv run pyright propstore`
- Pass: `powershell -File scripts/run_logged_pytest.ps1 -Label rule-proposal-artifact-rename tests/test_promote_rules_proposals.py tests/test_proposal_rules_family.py`
  - Evidence: `logs/test-runs/rule-proposal-artifact-rename-20260526-010120.log`,
    5 passed.
...
## Iteration 5 - `predicate proposal document surface`
...
- `PredicateProposalDocument`
  - Disposition: delete
  - Owner after cleanup: `PredicateProposalArtifact`, generated by
    `PREDICATE_PROPOSAL_CHARTER`.
  - Action: Replaced the handwritten persisted predicate proposal container
    with a generated charter-backed artifact that keeps the same YAML fields
    and placement.
- Predicate proposal contract manifest entries
  - Disposition: rewrite
  - Owner after cleanup: generated contract manifest plus explicit compatible
    change markers.
  - Action: Regenerated the checked-in semantic contract manifest and marked
    the proposal predicate family/artifact body changes compatible because the
    persisted YAML fields and placement did not change.

Gate results:
- Pass: `rg -n -F -- "PredicateProposalDocument" propstore tests`
- Pass: `uv run pyright propstore`
- Pass: `powershell -File scripts/run_logged_pytest.ps1 -Label predicate-proposal-artifact tests/test_promote_predicates_proposals.py tests/test_proposal_predicates_family.py tests/test_contract_manifest.py`
  - Evidence: `logs/test-runs/predicate-proposal-artifact-20260526-010756.log`,
    21 passed.
