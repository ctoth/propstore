...
- Test deletion commit `07a7c634` removed
...
- Test commit `c1e58dbe` migrated
  `tests/remediation/phase_7_race_atomicity/test_T7_5d_promotion_blocked_id_collision.py`
  from `connect_sqlite_store` to Quire SQLAlchemy `readonly_session` over
  `world_sqlalchemy_schema()`.
- The same test update removed the stale claim-row mirror assertion:
  `compile_promotion_blocked_models` now returns diagnostics only, so the
  current semantic coverage is that source-local promotion-blocked facts do
  not create `claim_core` rows and duplicate branch diagnostics remain
  preserved.
- `rg -n -F -- "connect_sqlite_store"
  tests/remediation/phase_7_race_atomicity/test_T7_5d_promotion_blocked_id_collision.py`
  returned zero hits.
