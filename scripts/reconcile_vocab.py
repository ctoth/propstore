...
MERGES = [
    ...
    (
        "last_link_ordering",
        "last_link_ordering_full",
        "Modgil 2014 vs 2018 naming - same ordering principle, '_full' is label artifact",
    ),
    (
        "weakest_link_ordering",
        "weakest_link_ordering_full",
        "Modgil 2014 vs 2018 naming - same ordering principle, '_full' is label artifact",
    ),
    (
        "contrariness_function",
        "contrariness_function_generalization",
    ...
    # TMS node types
    ("tms_node", "in_node", "NOT MERGING - in_node is a specific state of a tms_node"),
]
...
KEPT_DISTINCT = [
    ...
    (
        ...
        "statistical_syllogism_undercutter",
    ...
    (
        "dempster_rule",
        "dempster_rule_simplified",
        "Full Dempster's rule vs simplified version for BMS",
    ),
    (
        "rationality_postulate_direct_consistency",
        "rationality_postulate_indirect_consistency",
    ...
    (
        "argument_component",
        "argument_component_detection",
        "The NLP entity type vs the detection task",
    ),
    ("claim", "claim_lineage", "A claim vs the provenance chain of a claim"),
    (
        "rct_dataset",
        "rct_dataset_statistics",
        "The dataset vs descriptive statistics about it",
    ),
    (
        "sequence_tagging",
        "sequence_tagging_hyperparameters",
            ...
            if param_field in claim and isinstance(claim[param_field], list):
                for param in claim[param_field]:
                    if isinstance(param, dict) and "concept" in param:
                        if (
                            isinstance(param["concept"], str)
                            and param["concept"] in alias_to_canonical
                        ):
