...
| WS-M | Provenance — Trusty URI / PROV-O / gunray boundary / truncated-hash audit | CLOSED 932bb92a | E, **CM**, **O-gun** | — | 12 (T4.1-T4.7, Codex #27 + cluster M HIGH/MED) + truncated-hash sweep | consumes D-7 through WS-CM; **D-20 (truncated-hash audit — full content-hash identity, truncation display-only)**; migration command deleted per D-3 (Codex 1.4); single PROV-O path generated from one internal domain model (Codex 1.5); `return_arguments=True` default now consumes WS-O-gun budget wiring; report: `reviews/2026-04-26-claude/workstreams/reports/WS-M-closure.md` |
