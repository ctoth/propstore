...
| WS-G | AGM/iterated/IC postulate coverage | CLOSED e2f6bd62 | D, O-arg | — | 11 (T2.2, T2.6 + cluster C HIGH/MED + Codex #18-#20, #23) | **D-12 (AGM-AF stays in argumentation pkg)**; coverage-only labels separated from must-fail (Codex 2.4); deterministic stale-cache fixture (Codex 2.5); paper-derived OCF worked example (Codex 2.6); upstream argumentation no-stable fix `de6b66f`; report: `reviews/2026-04-26-claude/workstreams/reports/WS-G-closure.md` |
| WS-O-gun | gunray fixes + anytime wire-up | CLOSED v0.1.0 | — | M, H | 17 (cluster R HIGH/MED/LOW + ARCH/BND) | **D-17 (Garcia rewrite split off), D-18 (EnumerationExceeded wired)**; closed by pushed gunray tag `v0.1.0` at `27b0a13`; propstore pin and consumer budget surfacing remain WS-M gates; report: `reviews/2026-04-26-claude/workstreams/reports/WS-O-gun-closure.md` |
| WS-H | Probabilistic argumentation | CLOSED dd69f2bd | D, **O-gun**, **O-arg-gradual** | — | 9 (Codex #14-#17 + cluster F HIGH/MED) | Routed paper-TD complete; kept uncalibrated topology with omission diagnostics; stopped raw-confidence evidence fabrication; added Brier/log-loss, global sensitivity, COH convergence result, immutable omissions. Expected skips remain for WS-M budget consumer flip and DF-QuAD successor assertion; report: `reviews/2026-04-26-claude/workstreams/reports/WS-H-closure.md` |
| **WS-O-gun-garcia** | gunray Garcia 2004 generalized specificity rewrite | CLOSED 7caf63a0 | O-gun | M | new spec; replaces `not_defeasibly` semantics | **D-17 path**; Garcia surface is exactly `yes/no/undecided/unknown`; pushed gunray `64c4f3d340e0ac54651a9e5dd94bdf4df3d6cb7e`; propstore pin/gates through `7caf63a0`; report: `reviews/2026-04-26-claude/workstreams/reports/WS-O-gun-garcia-closure.md` |
| WS-K | Heuristic discipline, layer-3 boundary, embedding identity | CLOSED 3c40537b | E | N2 | 18 (T3.1, T5.1-T5.2 + cluster H HIGH/MED + cluster U layering) + H10/H11 + embedding identity | **D-8 (proposal_source_trust DROPPED; argumentation pipeline replaces)**; **D-19 (embedding model-key collisions — typed `EmbeddingModelIdentity` tuple)**; H10/H11 in-scope, no deferral (Codex 1.16); report: `reviews/2026-04-26-claude/workstreams/reports/WS-K-closure.md` |
| **WS-K2** | Meta-rule extraction pipeline | CLOSED 2f76cb38 | K | — | new spec; LLM extract → `proposal_rules` → human gate | **D-9 (LLM + proposal gate), D-10 (`knowledge/rules/<paper-name>/`)**; predicate-registration step explicit, no missing-skill reference (Codex 2.13); CLI tests for help/dry-run/propose/promote/unknown-id/no-commit-review (Codex 2.14); report: `reviews/2026-04-26-claude/workstreams/reports/WS-K2-closure.md` |
| WS-L | Merge non-commitment & sameAs | CLOSED e69f1d09 | E | — | 10 (T3.3-T3.4 + cluster I HIGH/MED) | non-collapse merge semantics; report: `reviews/2026-04-26-claude/workstreams/reports/WS-L-closure.md` |
| WS-M | Provenance — Trusty URI / PROV-O / gunray boundary / truncated-hash audit | CLOSED 623f892d | E, **CM**, **O-gun** | — | 12 (T4.1-T4.7, Codex #27 + cluster M HIGH/MED) + truncated-hash sweep | consumes D-7 through WS-CM; **D-20 (truncated-hash audit — full content-hash identity, truncation display-only)**; migration command deleted per D-3 (Codex 1.4); single PROV-O path generated from one internal domain model (Codex 1.5); `return_arguments=True` default now consumes WS-O-gun budget wiring; sidecar input persistence uses explicit JSON rather than pickle; source-branch hash contract manifest bumped; report: `reviews/2026-04-26-claude/workstreams/reports/WS-M-closure.md` |
...
| **WS-N1** | Architecture immediate (no WS-K dependency) | CLOSED 41b1fdf7 | — | — | 5 (T0.3 partial, Codex #38-#40 CLI extraction, doc-drift, dead pyproject entry, `_canonical_json` collapse) | **D-3 (delete all shims), D-4 (WorldModel→WorldQuery via rope), D-5 (Verdict→GroundedClassification)**; **D-26 immediate side** — CLI ownership extraction, shim deletion, two renames, three duplicate `_canonical_json` collapse to `quire.hashing`, doc-drift sweep; report: `reviews/2026-04-26-claude/workstreams/reports/WS-N1-closure.md` |
| **WS-N2** | Architecture layered (final six-layer contract) | CLOSED 05f9f482 | K, N1 | — | 3 (T5.3-T5.6, T5.8 final layered-contract enforcement; T0.3 negative-violation test) | **D-26 layered side** — final six-layer import-linter contract passes hard-clean; negative injection rejects every lower-to-higher layer pair; no allowlist or forbidden spot-check contracts remain; report: `reviews/2026-04-26-claude/workstreams/reports/WS-N2-closure.md` |
| WS-O-bri | bridgman fixes — `verify_equation` deleted | CLOSED 69241ae1 | — | P | 8 (cluster Q HIGH/MED + missing) | **D-13 (route to symbolic)**; deletion now, no deprecation period (Codex 2.27); `uv run` scripts replace bare `python -c` (Codex 2.28); pushed bridgman `d0f20b5`; report: `reviews/2026-04-26-claude/workstreams/reports/WS-O-bri-closure.md` |
...
| **WS-P2** | Condition semantics surface | CLOSED c5464445 | P, N2 | — | successor spec; CEL frontend boundary, checked condition carrier, solver deletion, sidecar/runtime representation, architecture docs | delete `propstore.z3_conditions` and `propstore.cel_checker`; replace `CheckedCel*` production carriers with `CheckedConditionSet` / `ConditionIR`; confine raw `z3` imports to the condition solver/backend boundary; docs gate covers `docs/cel-typed-expressions.md`, `docs/conflict-detection.md`, `docs/data-model.md`, `docs/python-api.md`, and architecture boundary docs; report: `reviews/2026-04-26-claude/workstreams/reports/WS-P2-closure.md` |
| **WS-J2** | InterventionWorld (Pearl do, Halpern HP-modified) | CLOSED e4207ff5 | D, J | — | new spec; AC1/AC2/AC3 + SCM data structure | **D-11 path B**; **D-22** — feature/research stream, not on critical bug-fix path; escalates if downstream reliance discovered; report: `reviews/2026-04-26-claude/workstreams/reports/WS-J2-closure.md` |
