...
2. **`tests/test_intervention_world_distinct_from_observation.py`** (new)
   ...
   - Assert the deterministic-inconsistent case raises `ObservationInconsistent` (NOT a silent fall-through to overlay or a Bayesian update — full Bayesian observation is out of scope for WS-J2 per Codex re-review #19; tracked as future WS-J3).
...
**Out of scope for WS-J2 (tracked for future WS-J3)**: full Bayesian observation, including probabilistic conditioning over `P(U)`, posterior updates over endogenous values, soft evidence, and any observation that does not reduce to a deterministic-consistency check. Once `Opinion`-typed `prior_base_rate` lands (D-8) and probabilistic SCM extension (Pearl 2000 Ch 7) is in place, WS-J3 will replace this deterministic-only cut with full Bayesian observation. WS-J2 does NOT promise that capability and does NOT block on it.
...
- `docs/worldlines.md` — add a new section "Counterfactual primitives" explaining the three world types: `OverlayWorld` (synthetic claim, conflict-resolution adjudication), `ObservationWorld` (deterministic-only observation / not-intervention; no edge severance; Bayesian conditioning deferred to WS-J3), `InterventionWorld` (Pearl do(), edge severance, SCM-level surgery). Include the canonical Suzy/Billy worked example as a Halpern 2015 §4 reference.
