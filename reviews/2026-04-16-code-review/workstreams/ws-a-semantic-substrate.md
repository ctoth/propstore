...
Date: 2026-04-16
Status: complete 2026-04-17 after post-upgrade full-suite gate
...
- 2026-04-17: WS-A final gate completed after `uv sync --upgrade`. Upgrade moved `curies` 0.13.3 -> 0.13.4, `pydantic` 2.13.1 -> 2.13.2, and `pydantic-core` 2.46.1 -> 2.46.2. The first full suite reached `2609 passed` before an xdist worker crashed on `tests/test_aspic_bridge.py::TestBridgeRationalityPostulates::test_direct_consistency`; the isolated rerun of that exact test passed (`1 passed`, `logs\test-runs\aspic-direct-consistency-isolate-20260417-050431.log`). The final logged full-suite run passed: `2610 passed` in `logs\test-runs\ws-a-final-green-3-20260417-050512.log`. The installed Pyright CLI still rejects `--strict`, and broader touched-surface Pyright runs still expose existing typing debt recorded above, so this closeout does not claim a new repository-wide type-clean state beyond the targeted passing Pyright slices already listed.
